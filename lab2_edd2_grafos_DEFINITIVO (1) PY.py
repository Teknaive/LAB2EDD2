# -*- coding: utf-8 -*-
"""LAB2_EDD2_GRAFOS (1).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/14hX6vdLI5JA4luIJ1DXtb355Z_oHqCpR
"""
...
def visualizar_aeropuertos_folium(codigos_aeropuertos, coordenadas_aeropuertos, info_aeropuertos):
    """
    Visualiza los aeropuertos en un mapa interactivo usando Folium, mostrando su geolocalización exacta.

    Args:
    codigos_aeropuertos (List[str]): Lista de códigos de aeropuertos.
    coordenadas_aeropuertos (List[Tuple[float, float]]): Lista de coordenadas de aeropuertos.
    info_aeropuertos (List[Dict[str, str]]): Información detallada de cada aeropuerto.

    Returns:
    None: Guarda el mapa como un archivo HTML.
    """
    # Crear un mapa centrado en el promedio de todas las coordenadas
    lat_promedio = sum(lat for lat, _ in coordenadas_aeropuertos) / len(coordenadas_aeropuertos)
    lon_promedio = sum(lon for _, lon in coordenadas_aeropuertos) / len(coordenadas_aeropuertos)
    mapa = folium.Map(location=[lat_promedio, lon_promedio], zoom_start=3)

    # Crear un grupo de marcadores para los aeropuertos
    marker_cluster = MarkerCluster().add_to(mapa)

    # Agregar marcadores para cada aeropuerto
    for codigo, (lat, lon), info in zip(codigos_aeropuertos, coordenadas_aeropuertos, info_aeropuertos):
        popup_text = f"""
        <b>{info['nombre']} ({codigo})</b><br>
    ...
    mapa.save("mapa_aeropuertos_geolocalizados.html")
    print("Mapa interactivo con geolocalización guardado como 'mapa_aeropuertos_geolocalizados.html'")
...
def main():
    ...
    while True:
        ...
        print("6. Mostrar Mapa Detallado")
        print("7. Salir")
        print("----------------------------------------")
        ...
        if opcion == '1':
            ...
            visualizar_aeropuertos_folium(codigos_aeropuertos, coordenadas_aeropuertos, info_aeropuertos)
            print("----------------------------------------")
        elif opcion == '7':
